...
@register_model("tf_gcn")
class GCN(base_model.BaseModel):
    @staticmethod
    def add_args(parser):
        ...
        parser.add_argument("--early-stopping", type=int, default=10, help="Tolerance for early stopping (# of epochs).")
        parser.add_argument("--max-degree", type=int, default=3, help='Maximum Chebyshev polynomial degree.')
        # fmt: on
    ...
    def __init__(self, graph, learning_rate=0.01, epochs=200,
        ...
        # Create model
        self.model = GCN(
            self.placeholders, input_dim=self.features[2][1], hidden1=self.hidden1, weight_decay=self.weight_decay,
            logging=True)
        # Initialize session
